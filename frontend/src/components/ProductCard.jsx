...
function ProductCard({ product, onClick, onDelete, onEdit, initialWishlisted = false, onWishlistToggle }) {
  ...
  return (
    <div onClick={onClick} className="bg-white dark:bg-gray-800 relative rounded-xl overflow-hidden cursor-pointer group border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow flex flex-col h-full">
          ...
          <div className="w-full h-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center">
            {imgError ? (
              <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-gray-400">
                ...
                <line x1="17.5" y1="14.6" x2="9.4" y2="6.5" />
          ...
          {user && (
            <button
              ...
              className={`p-2 rounded-full transition-colors border ${
                ...
                : 'bg-white/90 dark:bg-gray-800/90 text-gray-400 dark:text-gray-500 hover:text-red-500 border-gray-200 dark:border-gray-700 hover:bg-white'
              }`}
            >
          ...
          {user?.id === product.seller_id && (
            <div className="flex flex-col gap-2">
              <button
                ...
                className="p-1.5 sm:p-2 bg-green-500/90 hover:bg-green-600 rounded-full text-white transition-colors"
                title="Edit Produk"
              >
              ...
              <button
                ...
                className="p-1.5 sm:p-2 bg-red-500/90 hover:bg-red-600 rounded-full text-white transition-colors"
      ...
      <div className="p-2 flex flex-col flex-1">
        ...
        <div className="hidden sm:flex items-center gap-1.5 mb-2 text-xs text-gray-500 dark:text-gray-400">
          <Star size={11} className={`${(product.avg_rating || 0) > 0 ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300'}`} />
          <span className="font-medium">{(product.avg_rating || 0) > 0 ? (product.avg_rating || 0).toFixed(1) : '-'}</span>
